                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <img src="{{ asset('storage/img-produk/' . $show->foto) }}" class="img-fluid rounded mb-3" width="100%">
                            </div>
                            ...
                            <div class="form-group">
                                    ...
                                    <div class="row">
                                        @foreach($show->fotoProduk as $gambar)
                                            <div class="col-12 mb-2">
                                                <img src="{{ asset('storage/img-produk/' . $gambar->foto) }}" class="img-fluid border" width="100%">
                                                <form action="{{ route('backend.foto_produk.destroy', $gambar->id) }}" method="post" class="mt-1">
                                                    @csrf
                                                    @method('delete')
                                ...
                                <button type="button" class="btn btn-primary btn-sm add-foto mt-2">Tambah Foto</button>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addFotoButton.addEventListener('click', function() {
            ...
            fotoRow.classList.add('form-group', 'row', 'mt-2'); 
            fotoRow.innerHTML = `
                <form action="{{ route('backend.foto_produk.store') }}" method="post" enctype="multipart/form-data" class="w-100">
                    @csrf
                    <div class="col-12">
                        <input type="hidden" name="produk_id" value="{{ $show->id }}">
                        <div class="d-flex">
                            <input type="file" name="foto_produk[]" class="form-control mr-2 @error('foto_produk') is-invalid @enderror">
